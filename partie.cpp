...
void init(Partie& partie, char pl[]) {
  ...
  // pl est la chaine de caract�res pass�e en param�tre de la fonction, et qui correspond � argv[1]
 ...
 * @param[in] partie : La partie en cours
 * @param[in] lettre : caractere que le joueur a saisi
...
/**
 * @brief Ajouter la lettre saisi au mot de la partie
 * @param[in-out] partie : La partie en cours
 * @param[in] lettre : La lettre a ajouter au mot.
 */
...
void mancheJeu(Dico& idico, Partie& partie) {
  int indexmotdudico = 0;
  while (partie.taillemot <= MAX_LENMOT) {
    for (unsigned int i = 0; i < partie.nbJoueurs; ++i) {
      ...
      case 1:
        // si le joueur a entr� '?'
        // saisirMotHumain renvoie la chaine de caract�res �crit par le joueur
        saisirMot(idico, partie, i);
...
/**
 * @brief Saisir un mot si le joueur pr�c�dent a saisi '?'
 * @param[in] idico : Le dictionnaire
 * @param[in] partie : La partie en cours
