...
/**
 * @brief Trouver un mot pour le robot si le joueur pr�c�dent a entr� '?', et l'afficher
...
/**
 * @brief G�n�rer une lettre al�atoire si le robot joue en premier
 * @param[in-out] partie: La partie en cours
 */
...
int construireMot_Robot(Dico& idico, Partie& partie, char mot[], unsigned int& nbNombre, char* listeLettre) {
  ...
  int nbRand = 0;
  // lettreExiste == 1 si la lettre a deja �t� g�n�r�e
  while (lettreExiste) {
    if (nbRand == MAX_CHAR)
      return 1;
